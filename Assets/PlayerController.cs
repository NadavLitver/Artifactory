...
public class PlayerController : MonoBehaviour
{
    ...
    //[SerializeField] LiveBody m_livebody;
    ...
    {
        if ((isGrounded || CoyoteAvailable) && InputManager.instance.JumpDown() && canMove)
        {
            Jumping = true;
    ...
    void SetAnimatorParameters()
    {
        ...
        //m_livebody.animator.SetBool("Grounded", isGrounded);
        //m_livebody.animator.SetBool("Running", (horInput != 0 && isGrounded));
        isFalling = (!isGrounded && m_rb.velocity.y < -1f);
        //m_livebody.animator.SetBool("Falling", isFalling);
    }
    private void SetVelocity()
    {
        horInput = InputManager.instance.GetMoveVector().x;
        if (canMove)
        {
            //check flip
            if (horInput > 0)
